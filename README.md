...
│   │                            # - Configurable pixelation levels (default: 15 levels from 1.0 to 10.0)
│   │
...
│   └── EDA.ipynb                
│
├── pixelated_mnist/             # Generated pixelated images (200 images × 15 levels)
...
**Key Features**:
- `Pixelate` class: PyTorch module that applies pixelation via downscaling
- `apply_pixelation_to_mnist()`: Main function that processes images
- Configurable pixelation levels (currently 15 levels: 1.0, 1.2, 1.4, ..., 10.0)
- Creates organized directory structure: `pixelated_mnist/image_XXX_class_Y/`
...
1. **Generate pixelated images**: Run `src/apply_pixel_levels.py` to create pixelated versions after grabbing MNIST/EMNIST Data
2. **Run experiments**: Use `src/run_experiment.py` to collect human perception data
3. **Analyze results**: Use notebooks or custom scripts to analyze `human_data/*/results.csv`